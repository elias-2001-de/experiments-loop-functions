/**
  * @file <loop-functions/sugar/AggregationLoopFunc.cpp>
  *
  * @author Antoine Ligot - <aligot@ulb.ac.be>
  *
  * @package ARGoS3-AutoMoDe
  *
  * @license MIT License
  */

#include "AggregationLoopFunc.h"

/****************************************/
/****************************************/

AggregationLoopFunction::AggregationLoopFunction() {
  m_fRadius = 0.3;
  m_cCoordBlackSpot = CVector2(0,0.6);
  m_cCoordWhiteSpot = CVector2(0,-0.6);
  m_fObjectiveFunction = 0;
}

/****************************************/
/****************************************/

AggregationLoopFunction::AggregationLoopFunction(const AggregationLoopFunction& orig) {}

/****************************************/
/****************************************/

AggregationLoopFunction::~AggregationLoopFunction() {}

/****************************************/
/****************************************/

void AggregationLoopFunction::Destroy() {}

/****************************************/
/****************************************/

void AggregationLoopFunction::Init(argos::TConfigurationNode& t_tree) {
	CoreLoopFunctions::Init(t_tree);
	CSpace::TMapPerType& tEpuckMap = GetSpace().GetEntitiesByType("epuck");
	int i = 0;
	for (CSpace::TMapPerType::iterator it = tEpuckMap.begin(); it != tEpuckMap.end(); ++it) {
		CEPuckEntity* pcEpuck = any_cast<CEPuckEntity*>(it->second);
		std::string str = pcEpuck->GetId();

		// Find the position of the substring patch if it is in the ID
    		size_t pos = str.find(to_find);

		if (pos != std::string::npos) {
			MoveEntity(pcEpuck->GetEmbodiedEntity(),
					list_of_patch_positions.at(i),
				       	CQuaternion().FromEulerAngles(m_pcRng->Uniform(CRange<CRadians>(CRadians::ZERO,CRadians::TWO_PI)),
                            CRadians::ZERO,CRadians::ZERO),false);
      
      // Set the color of the patch to the controller
      AutoMoDePatchController& controller = dynamic_cast<AutoMoDePatchController&>(pcEpuck->GetControllableEntity().GetController());
      LOG << "Which color to send? " << list_of_colors.at(i) << std::endl;
      controller.SetColor(list_of_colors.at(i));

      i += 1;
		}
	}
}	
		
/****************************************/
/****************************************/

void AggregationLoopFunction::Reset() {
  m_fObjectiveFunction = 0;
  m_unScoreSpot = 0;
  CoreLoopFunctions::Reset();

  CSpace::TMapPerType& tEpuckMap = GetSpace().GetEntitiesByType("epuck");
  int i = 0;
  for (CSpace::TMapPerType::iterator it = tEpuckMap.begin(); it != tEpuckMap.end(); ++it) {
	  CEPuckEntity* pcEpuck = any_cast<CEPuckEntity*>(it->second);
          std::string str = pcEpuck->GetId();

          // Find the position of the substring patch if it is in the ID
          size_t pos = str.find(to_find);

          if (pos != std::string::npos) {
		  MoveEntity(pcEpuck->GetEmbodiedEntity(),
				  list_of_patch_positions.at(i),
                                  CQuaternion().FromEulerAngles(m_pcRng->Uniform(CRange<CRadians>(CRadians::ZERO,CRadians::TWO_PI)),
					  CRadians::ZERO,CRadians::ZERO),false);
		  
      // Set the color of the patch to the controller
      AutoMoDePatchController& controller = dynamic_cast<AutoMoDePatchController&>(pcEpuck->GetControllableEntity().GetController());
      controller.SetColor(list_of_colors.at(i));
      
      i += 1;
          }
  }
}


/****************************************/
/****************************************/

argos::CColor AggregationLoopFunction::GetFloorColor(const argos::CVector2& c_position_on_plane) {
  CVector2 vCurrentPoint(c_position_on_plane.GetX(), c_position_on_plane.GetY());
  Real d = (m_cCoordBlackSpot - vCurrentPoint).Length();
  if (d <= m_fRadius) {
    return CColor::BLACK;
  }

  d = (m_cCoordWhiteSpot - vCurrentPoint).Length();
  if (d <= m_fRadius) {
    return CColor::WHITE;
  }


  return CColor::GRAY50;
}

/****************************************/
/****************************************/

void AggregationLoopFunction::PostStep() {
  CSpace::TMapPerType& tEpuckMap = GetSpace().GetEntitiesByType("epuck");
  CVector2 cEpuckPosition(0,0);
  int i = 0;
  for (CSpace::TMapPerType::iterator it = tEpuckMap.begin(); it != tEpuckMap.end(); ++it) {
	  CEPuckEntity* pcEpuck = any_cast<CEPuckEntity*>(it->second);
          std::string str = pcEpuck->GetId();

          // Find the position of the substring patch if it is in the ID
          size_t pos = str.find(to_find);

          if (pos == std::string::npos) {
    		  cEpuckPosition.Set(pcEpuck->GetEmbodiedEntity().GetOriginAnchor().Position.GetX(),
                       pcEpuck->GetEmbodiedEntity().GetOriginAnchor().Position.GetY());
		  Real fDistanceSpot = (m_cCoordBlackSpot - cEpuckPosition).Length();
    		  if (fDistanceSpot <= m_fRadius) {
			  m_fObjectiveFunction += 1;
		  }
    	  } else {
		  MoveEntity(pcEpuck->GetEmbodiedEntity(),
                                  list_of_patch_positions.at(i),
                                  CQuaternion().FromEulerAngles(m_pcRng->Uniform(CRange<CRadians>(CRadians::ZERO,CRadians::TWO_PI)),
                                          CRadians::ZERO,CRadians::ZERO),false);
		  i += 1;
	  }
  }
  LOG << m_fObjectiveFunction << std::endl;
}


/****************************************/
/****************************************/

Real AggregationLoopFunction::GetObjectiveFunction() {
  return m_fObjectiveFunction;
}

/****************************************/
/****************************************/

CVector3 AggregationLoopFunction::GetRandomPosition() {
  Real a;
  Real b;
  Real temp;

  a = m_pcRng->Uniform(CRange<Real>(0.0f, 1.0f));
  b = m_pcRng->Uniform(CRange<Real>(0.0f, 1.0f));
  // If b < a, swap them
  if (b < a) {
    temp = a;
    a = b;
    b = temp;
  }
  Real fPosX = b * m_fDistributionRadius * cos(2 * CRadians::PI.GetValue() * (a/b));
  Real fPosY = b * m_fDistributionRadius * sin(2 * CRadians::PI.GetValue() * (a/b));
  return CVector3(fPosX, fPosY, 0);
}

REGISTER_LOOP_FUNCTIONS(AggregationLoopFunction, "aggregation_loop_functions");
